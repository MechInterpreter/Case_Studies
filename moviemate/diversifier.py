...
def calculate_gini_index(selected_indices, genre_vectors=None):
    """
    ...
    if len(selected_indices) == 0:
        return 0  # No items selected, Gini index is 0
    
    counts = pd.Series(selected_indices).value_counts().values
    n = len(counts)
    
    if n <= 1:
        return 0  # Gini index is undefined or irrelevant for single-item lists; return 0 to be safe
    
    # Sort counts to compute cumulative values
    sorted_counts = np.sort(counts)
    cumulative_sum = np.cumsum(sorted_counts)
    
    # Prevent division by zero if cumulative sum is zero
    if cumulative_sum[-1] == 0:
        return 0
    
    # Calculate Gini Index with stability checks
    gini_value = (2 / n) * np.sum((np.arange(1, n + 1) * sorted_counts)) / cumulative_sum[-1] - (n + 1) / n
    
    # Ensure the Gini Index value is non-negative and not above 1
    gini_value = max(0, min(gini_value, 1))
